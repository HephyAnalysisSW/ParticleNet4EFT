import torch
from utils.nn.model.ParticleNet_Lena import ParticleNetTagger
from torch import Tensor
import math

def get_model(data_config, **kwargs):
    conv_params =  [(1, (3,3,3))]  # Note to myself: first parameter in conv_params is (probably) the number of connected neighbours
    fc_params = [(2, 0.0)]           # fully connected layer (hidden_size, drop_out) after GNN for array based input
    fc_global_params = [(40, 0.0)]   # fully connected layer (hidden_size, drop_out) for global features only
