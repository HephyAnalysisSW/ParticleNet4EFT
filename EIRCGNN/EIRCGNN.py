...
class EdgeConv(MessagePassing):
    ...
    def message(self, x_i, x_j):
        ...
        # x_i, x_j: (pt[1], features[N], angles[2])
        ...
        #print (angles_i.shape)
        #print (angles_j.shape)
        
        #tmp = torch.cat([x_i[:,1:-2], x_j[:,1:-2], x_j[:,1:-2]-x_i[:,1:-2],  angles_ij], dim=1)  # tmp has shape [E, 2 * in_channels]
        ##return self.mlp(tmp)
        #print ("EC input", tmp.shape, tmp.dtype)
        #res = self.mlp(tmp)
        #print ("EC output", res.shape)
        return torch.cat(  
                    (x_i[:,:1], #return pt of xi and mlp output ... 
                     self.mlp(torch.cat(
                            [x_i[:,1:-2], # features of node i
                             x_j[:,1:-2], # features of node j
                             x_j[:,1:-2]-x_i[:,1:-2], # differences
                             angles_ij], dim=1)), #sin / cos(gamma_i - gamma_j)
                     x_i[:,-2:], # ... and finally the angles_norm of xi
                     ), dim=1 ) 
        #print ("message[0]:", result[0])
        #return result

    def aggregate( self, inputs, index):#,  **kwargs):
        #print ("Called aggregate. Inputs/index:", inputs.shape, index.shape)

        #print ("Aggregate:", inputs[0] )
        ...
        #NN_features = inputs[:,1:-1]
        #NN_gamma    = inputs[:,-1:]
        #print ("Aggregate: pt",pt.shape)
        #print ("Aggregate: NN_features",NN_features.shape)
        #print ("Aggregate: NN_gamma",NN_gamma.shape)
        #result = super().aggregate( inputs, index)
        #print ("Aggregate: result", result.shape)
        #print ("result[0]", result[0])
        #print ("Agrees?  ", inputs[index==0].sum(dim=0))
        #return result

        #print ( "Shape", (wj.view(-1,1)*inputs).shape )

        # first, we add up (weighted) the pt [0], the MLP [1,...Nf+1], and the two angles
        result = torch.zeros((len(index.unique()),inputs.shape[1]),dtype=torch.float).index_add_(0, index, wj.view(-1,1)*inputs)
        #print ("result", result.shape)
        #print ("angles_norm?", result[:,-2:])

        # Now we rotate the angle (last two columns) by the last MLP output (at position -3).
        # We return pt, the features (up to position -3) and append the rotated angles in the last two columns
...
class EIRCGNN(EdgeConv):
    def __init__(self, mlp, dRN=0.4, max_num_neighbors=-1):
        super().__init__(mlp=mlp)
        self.dRN = dRN
        self.max_num_neighbors = max_num_neighbors

    def forward(self, pt, features, angles, batch):
        #rho        = torch.view_as_complex( angles ).abs()
        #angles_norm= angles_train/rho.view(-1,1)
        edge_index = radius(angles, angles, r=self.dRN, batch_x=batch, batch_y=batch, max_num_neighbors=self.max_num_neighbors)
...
class Net(torch.nn.Module):
    def __init__(self, num_classes=num_classes, conv_params=conv_params, dRN=0.4, max_num_neighbors=-1, readout_params=readout_params):
        super().__init__()
        ...
        for l, (dropout, hidden_layers) in enumerate(conv_params):
            ...
            if l==0:
                self.EC.append( EIRCGNN(MLP([3 + 2 ]+hidden_layers_, dropout=dropout, act="LeakyRelu"), dRN=dRN, max_num_neighbors=max_num_neighbors ) )
            else:
                self.EC.append( EIRCGNN(MLP([3*conv_params[l-1][1][-1]+2]+hidden_layers_,dropout=dropout, act="LeakyRelu"), dRN=dRN, max_num_neighbors=max_num_neighbors ) ) 
